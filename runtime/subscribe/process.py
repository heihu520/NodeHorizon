...
def load_configs(
    ...
    def parse_config(config: dict) -> None:
        ...
        # Debug logging
        logger.info(f"[Debug] Parsed storage from config: engine={storage.get('engine')}, has_token={bool(storage.get('token'))}, has_items={bool(storage.get('items'))}")
    ...
    def verify(storage: dict, groups: dict) -> bool:
        ...
        # Debug logging
        logger.info(f"[Debug] Storage config: engine={storage.get('engine')}, has_token={bool(storage.get('token'))}, has_items={bool(storage.get('items'))}")
    ...
    try:
        if re.match(
            ...
            logger.info(f"[Debug] Looking for config file at: {localfile}")
            logger.info(f"[Debug] File exists: {os.path.exists(localfile)}, Is file: {os.path.isfile(localfile) if os.path.exists(localfile) else 'N/A'}")
            
            if os.path.exists(localfile) and os.path.isfile(localfile):
                config = json.loads(open(localfile, "r", encoding="utf8").read())
                os.environ["SUBSCRIBE_CONF"] = localfile
                logger.info(f"[Debug] Config file loaded successfully, has storage: {'storage' in config}")
                parse_config(config)
                
                # Inject GitHub Token from cloud.gistToken for crawler use
                cloud_config = config.get("cloud", {})
                gist_token = cloud_config.get("gistToken", "").strip()
                if gist_token and not os.environ.get("GH_TOKEN"):
                    os.environ["GH_TOKEN"] = gist_token
                    logger.info("[ConfigLoader] GitHub Token injected from config for crawler use")
            else:
                logger.error(f"[Debug] Config file not found or not accessible: {localfile}")
...
def aggregate(args: argparse.Namespace) -> None:
    ...
    for k, v in groups.items():
        ...
        for target, storage_name in targets.items():
            ...
            # Calculate subscription traffic info: 2TB total, 30 days expiration
            total_bytes = 2 * 1024 * 1024 * 1024 * 1024  # 2TB = 2199023255552 bytes
            expire_timestamp = int((datetime.now() + timedelta(days=30)).timestamp())
            userinfo = f"upload=1048576;download=2312456;total={total_bytes};expire={expire_timestamp}"
            ...
            success = subconverter.generate_conf(
                ...
                emoji=emoji,
                list_only=list_only,
                config="config/ACL4SSR_blackmatrix7.ini",
                userinfo=userinfo,
            )
            if not success:
            ...
            if subconverter.convert(binname=subconverter_bin, artifact=artifact):
                ...
                # Add userinfo comment for Clash YAML files
                logger.info(f"[UserinfoDebug] target={target}, userinfo={userinfo[:50] if userinfo else 'None'}, group={k}")
                if target == "clash" and userinfo:
                    userinfo_comment = f"# subscription-userinfo: {userinfo}\n"
                    content = userinfo_comment + content
                    logger.info(f"[UserinfoAdded] Added userinfo comment to {target} file, group: {k}")
                else:
                    logger.warning(f"[UserinfoSkipped] Skipped adding userinfo: target={target}, has_userinfo={bool(userinfo)}, group={k}")
